...
startup(){
  ...
  sudo chmod +x ${SHELL_FOLDER}/ngrokd
  sudo nohup ${SHELL_FOLDER}/ngrokd -tlsKey=/opt/ngrok/server.key \
                                    -tlsCrt=/opt/ngrok/server.crt \
                                    -domain=ngrok.magicarena.cn \
                                    -httpAddr=:7070 \
                                    -httpsAddr=:7071 \
                                    -tunnelAddr=:7443 \
                                    -log=stdout \
                                    -log-level=DEBUG \
                                    >> /var/log/ngrok/ngrokd.log &
}
